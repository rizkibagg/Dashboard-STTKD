-- #jumlah pendaftar perbulan - SUDAH
SELECT
    COUNT(1) AS total,
    CONCAT(
...
-- # jumlah catar by provinsi - SUDAH
SELECT
    COUNT(kkk.catarNoDaftar) AS total,
    kkk.provinsi
FROM
...
-- # Sumber informasi dari mana
SELECT truncate(((COUNT(*) * 100) / totalTaruna),1) as jumlah, aa.catarInfoSTTKD as keterangan FROM ptb_calon_taruna AS aa
CROSS JOIN (
    Select count(1) as totalTaruna
    from ptb_calon_taruna
    WHERE catarTahunDaftar="{{tahunRekap}}" and
    catarInfoSTTKD is not null
) as catar
JOIN ptb_tahun_ptb AS bb ON aa.catarTahunDaftar=bb.tahunPtb
WHERE bb.tahunPtb="{{tahunRekap}}"
GROUP BY aa.catarInfoSTTKD
